...
DijkstraShortestPath::DijkstraShortestPath(int row_a, int column_a, int row_b, int column_b, bool walls)
{
    ...
    // Initialize the map with walls (represented by 1)
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < columns; ++j) {
            ...
            {
                map[i][j] = get_rnd(0.20); // 20% chance that the cell will be a wall
            }
            else
            {
                map[i][j] = 0;
    ...
    adjacency_list_t adjacency_list(rows * columns + 1);  // Include an additional vertex for the walls
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < columns; ++j) {
            ...
            if (i > 0 && map[i - 1][j] != 1) {
                adjacency_list[i * columns + j].push_back(neighbor((i - 1) * columns + j, 1));  // Connect to the above cell
            }

            // Connect to the left cell
            if (j > 0 && map[i][j - 1] != 1) {
                adjacency_list[i * columns + j].push_back(neighbor(i * columns + j - 1, 1));  // Connect to the left cell
            }

            // Connect to the right cell
            if (j < columns - 1 && map[i][j + 1] != 1) {
                adjacency_list[i * columns + j].push_back(neighbor(i * columns + j + 1, 1));  // Connect to the right cell
            }
...
void DijkstraShortestPath::DijkstraComputePaths(vertex_t source, const adjacency_list_t& adjacency_list, std::vector<weight_t>& min_distance, std::vector<vertex_t>& previous)
{
    ...
    while (!vertex_queue.empty())
    {
        ...
        for (const neighbor& neighbor_iter : neighbors)
        {
            ...
            // Skip walls (nodes with weight = max_weight)
